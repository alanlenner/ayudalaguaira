...
const ZONAS = ["Todas", "Naiguatá", "Caraballeda", "Catia La Mar", "Maiquetía"] as const;
type Zona = (typeof ZONAS)[number];
type ZonaDB = Exclude<Zona, "Todas">;
...
function TarjetaReporte({ pub }: { pub: Reporte }) {
  ...
  const waLink = `https://wa.me/${telLimpio.startsWith("0") ? "58" + telLimpio.slice(1) : telLimpio}`;
    ...
    <div className="mt-8 mb-4">
      <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 mb-3">
        <p className="text-xs text-yellow-700 font-medium">
          Contenido externo de X, no verificado
        </p>
      </div>
...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!nombre.trim() || !apellido.trim() || !telefono.trim()) {
      alert("Completa nombre, apellido y teléfono de contacto");
      return;
    }
    ...
    if (fotoFile) {
      try {
        ...
        const ext = "jpg";
        const fileName = `${Date.now()}_${generarToken().slice(0, 8)}.${ext}`;
  ...
  const copiarLink = () => {
    ...
    const link = `${window.location.origin}/editar/${tokenGenerado}`;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="mt-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
            />
          </div>
        </div>
        ...
        <button
          ...
          className="w-full mt-3 bg-red-600 hover:bg-red-700 text-white py-3.5 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2 shadow-md"
        >
        ...
        <div className="mt-4 pb-8 space-y-3">
            ...
            <div className="text-center py-16">
              ...
              <p className="text-slate-500 font-medium">
                {busqueda
                  ...
                  : `No hay reportes${zonaActiva !== "Todas" ? ` en ${zonaActiva}` : ""}`}
              </p>
            </div>
          ) : (
            <>
              {reportes.map((r) => (
                <TarjetaReporte key={r.id} pub={r} />
              ))}
          ...
          {/* Twitter embed por zona */}
          {zonaActiva !== "Todas" && !busqueda && (
            <TwitterZona zona={zonaActiva} />
          )}
        </div>
      </div>
          ...
          <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b px-4 py-3 flex items-center justify-between z-10">
              <h2 className="font-bold text-lg">
                {tokenGenerado ? "Reporte publicado" : "Reportar desaparecido"}
              </h2>
              <button
            ...
            {tokenGenerado ? (
              /* Pantalla de éxito con link de edición */
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-700 mt-1">
                        Es el único modo de actualizar o marcar como encontrado
                        este reporte.
                      </p>
                    </div>
                  </div>
              ...
              <form onSubmit={handleSubmit} className="p-4 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      buscarDuplicados(e.target.value, apellido);
                    }}
                    placeholder="Nombre"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      buscarDuplicados(nombre, e.target.value);
                    }}
                ...
                {duplicados.length > 0 && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 space-y-2">
                    <p className="text-xs font-medium text-yellow-800">
                      Posibles reportes existentes:
                    </p>
                    {duplicados.map((d) => (
                      <div key={d.id} className="flex items-center justify-between">
                        <span className="text-xs text-yellow-700">
                          {d.nombre} {d.apellido} ({d.zona})
                        </span>
                        <a
                          href={`#reporte-${d.id}`}
                          onClick={() => setMostrarFormulario(false)}
                          className="text-xs text-yellow-600 underline font-medium"
                        >
                          Ver reporte
                        </a>
                      </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ej: Calle principal de Naiguatá, frente a la plaza"
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Contextura, ropa, señas particulares..."
                    rows={2}
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="encontrado_fallecido">
                      Encontrado fallecido
                    </option>
                  </select>
                </div>
